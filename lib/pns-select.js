    ...
    return {
        ...
        template: '<div class="pns-ng-select">' +
                        '<div class="select-input" ng-class="{\'text\': type==\'text\'}" ng-click="selectClick()">' +
                            '<input ng-model="item.name" ng-class="{\'form-disabled\':disabled}" ng-if="validate" class="input" type="text" pns-validate validate-regex="{{validateRegex}}" placeholder="{{input_placeholder}}" >'+
                            '<input ng-model="item.name" ng-class="{\'form-disabled\':disabled}" ng-if="!validate" class="input" type="text" placeholder="{{input_placeholder}}" >'+
                        '</div>' +
                        ...
                        for (i = 0; i < scope.selectList.length; i++) {
                            if(scope.parentKey){
                                ...
                                if (scope.selectList[i].value == val) {
                                    scope.item = scope.selectList[i];
                                    angular.element(element).find("input")[0].value = scope.selectList[i].name;
                                }
                            } 
                        }
                    }
