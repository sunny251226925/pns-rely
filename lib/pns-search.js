                                ...
                                '<tbody>' +
                                    ...
                                    '<tr ng-repeat="item in otherList">' +
                                        ...
                                        '<td class="search-content">' +
                                            ...
                                            '<div ng-if="item.type == \'MultipleList\' && !item.letterDisplay" pns-search-checkbox list="item.data" key="params[item.code]" parent-key="parentKey" reset-status="searchCheckboxReset" cascade="item.childDictTypeId" change="multipleListChange(item,cascade)"></div>' +
                                            '<div ng-if="item.type == \'DateRange\'" pns-date range="range" key="params[item.code]"></div>' +
                                            '<div ng-if="item.type == \'Date\'" pns-date key="params[item.code]"></div>' +
                                        '</td>' +
                                    '</tr>' +
            ...
            $rootScope.layPromise.then(function (layui) {
                ...
                scope.menuFilter = function(data){
                    ...
                    for(var i=0; i < otherList.length; i++){
                        ...
                        //合并Input
                        var size = scope.inputList.length > 0 ? 3 : 4;
                        if( i < size){
                ...
                scope.reset = function(){
                    ...
                    scope.searchCheckboxReset = Math.random();
                    for (var i = 0; i < deleteKey.length; i++) {
                        scope.params[deleteKey[i]] = "";
                    }
                    scope.inputListKey[scope.inputKeyFist] = "";
                    for(var keyName in scope.queryParams){
                        if(typeof scope.queryParams[keyName] == "object"){
                            scope.queryParams[keyName] = [];
                        } else {
                            scope.queryParams[keyName] = "";
                        }
                        if(keyName == ""){
                            delete scope.queryParams[keyName];
                        }
                    }

                    scope.params = scope.queryParams;

                    var childDictTypeIdList = [];

                    for(var i=0; i<scope.otherList.length; i++){
                        if(scope.otherList[i].childDictTypeId){
                            childDictTypeIdList = childDictTypeIdList.concat(scope.otherList[i].childDictTypeId.split(","));
                        }
                    }
                    for(var i=0; i<scope.moreList.length; i++){
                        if(scope.moreList[i].childDictTypeId){
                            childDictTypeIdList = childDictTypeIdList.concat(scope.moreList[i].childDictTypeId.split(","));
                        }
                    }
                    for(var i=0; i<scope.otherList.length; i++){
                        if(childDictTypeIdList.indexOf(scope.otherList[i].code) >=0 ){
                            for(var j=0; j<scope.otherList[i].data.length; j++){
                                scope.otherList[i].data[j].codeShow = false;
                            }
                        }
                    ...
                    for(var i=0; i<scope.moreList.length; i++){
                        if(childDictTypeIdList.indexOf(scope.moreList[i].code) >=0 ){
                            for(var j=0; j<scope.moreList[i].data.length; j++){
                                scope.moreList[i].data[j].codeShow = false;
                            }
                        }
