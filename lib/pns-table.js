module.exports = function(httpService, sessionFactory, $location){
    return {
        ...
        template: '<div class="table-box">'+
                    '<div class="operation">'+
                        '<i class="iconfont iconicon_table_listset" title="自定义列" ng-click="autoCloumnOpen()"></i>'+
                    '</div>'+
                    '<ul class="autoCloumn" ng-blur="autoCloumnBlur()" ng-show="autoCloumnShow" tabindex="1" id="{{autoCloumnId}}">'+
                        '<li class="item">'+
                            '<label class="label">'+
                                '<input type="checkbox" ng-mousedown="autoCloumnChangeAll($event,item)">'+
                                '<span>全选</span>'+
                            '</label>'+
                        '</li>'+
                        '<li class="item" ng-repeat="item in autoCloumnList" ng-if="!$index==0" ng-hide="$index==(autoCloumnList.length-1)"'+
                            '<label class="label">'+
                                '<input type="checkbox" ng-mousedown="autoCloumnChange($event,item)">'+
                    ...
                    '<div class="box">'+
                        '<table class="pns-table table">' +
                            '<thead class="thead">'+
                                '<tr class="tr">'+
                                    ...
                                    '<th class="th" ng-show="item.checked" ng-repeat="item in cols track by $index" ng-style="{width:item.width}">'+
                                        ...
                                        '<div class="title" ng-if="item.isType==\'button\'" ng-bind="item.title ? item.title : \'操作\'"></div>' +
                            ...
                            '</thead>' +
                            '<tbody class="tbody">' +
                                '<tr class="tr" ng-if="!list || list.length==0"><td class="td text-center" style="height:280px" colspan="{{cols.length+1}}">暂无数据</td></tr>' +
        ...
        link: function (scope, element, attr) {
            ...
            //自定义列-打开
            scope.autoCloumnOpen = function(){
                scope.autoCloumnShow = true;
                var selectCloumnList = JSON.parse(sessionFactory.get("selectCloumnList"));
                var curCloumn = selectCloumnList[path];
                scope.autoCloumnList = curCloumn;
                var falseSize = 0;
                setTimeout(function(){
                    $("#"+scope.autoCloumnId).focus();
                    for(var i=0;i<curCloumn.length; i++){
                        if(curCloumn[i].code){
                            $("#"+scope.autoCloumnId+" input[type=checkbox]").eq(i).prop("checked", curCloumn[i].checked);
                            if(!curCloumn[i].checked){
                                falseSize++
                            }
                        }
                    }
                    if(falseSize > 0){
                        $("#"+scope.autoCloumnId+" input[type=checkbox]").eq(0).prop("checked", false);
                    } else {
                        $("#"+scope.autoCloumnId+" input[type=checkbox]").eq(0).prop("checked", true);
                    }
                },200)
            }
            //自定义列-全选
            scope.autoCloumnChangeAll = function(event){
                ...
                for(var i=0; i<scope.autoCloumnList.length; i++){
                    ...
                    if(scope.autoCloumnList[i].code){
                        $("#"+scope.autoCloumnId+" input[type=checkbox]").eq(i).prop("checked", scope.autoCloumnList[i].checked); 
                    }
                }
            ...
            scope.autoCloumnBlur = function(){
                scope.autoCloumnShow = false;
                var selectCloumnList = JSON.parse(sessionFactory.get("selectCloumnList"));
                selectCloumnList[path] = scope.autoCloumnList;
                scope.cols = scope.autoCloumnList;
                sessionFactory.put("selectCloumnList",JSON.stringify(selectCloumnList));
            }

            //列过滤
            scope.cloumnFilter = function(cols,selectCloumnList){
                ...
                if(selectCloumnList){
                    selectCloumnList[path] = cols;
                } else {
                    selectCloumnList = {};
                    selectCloumnList[path] = cols;
                }
                scope.selectCloumnList = selectCloumnList;
            }

            //获取动态列
            scope.getSelectCloumnList = function(selectCloumnList){
                httpService.request({
                    method: "get",
                    ...
                    if (res.code == 200) {
                        scope.$watch("cloumn",function(){
                            scope.cloumnFilter(res.data,selectCloumnList);
                            sessionFactory.put("selectCloumnList", JSON.stringify(scope.selectCloumnList))
                        })
                    }
                })
            }

            scope.getCloumn = function(){
                var session = sessionFactory.get("selectCloumnList");
                var selectCloumnList = {};
                if(session){
                    selectCloumnList = JSON.parse(session);
                    if(selectCloumnList[path]){
                        scope.cols = selectCloumnList[path];
                    } else {
                        scope.getSelectCloumnList(selectCloumnList); 
                    }
                } else {
