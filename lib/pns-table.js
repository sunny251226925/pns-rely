module.exports = function(httpService, sessionFactory, $location){
    return {
        ...
        template: '<div class="table-box">'+
                    ...
                    '</div>'+
                    '<ul class="autoCloumn" ng-class="{true:\'open\',false:\'close\'}" ng-blur="autoCloumnBlur()" ng-show="autoCloumnShow" tabindex="1" id="{{autoCloumnId}}">'+
                        '<li class="item">'+
                            '<label class="label">'+
                    ...
                    '<div class="box">'+
                        '<table class="pns-table table">' +
                                ...
                                '<tr class="tr">'+
                                    ...
                                    '<th class="th" ng-show="item.checked" ng-repeat="item in cols track by $index" ng-style="{width:item.width}">'+
                                        '<div class="title" ng-show="item.isType==\'checkbox\'">'+
                                            '<input type="checkbox" ng-model="checkAll" ng-click="item.checkAll(item)" ng-show="item.isCheckAll">'+
                                        '</div>'+
                            ...
                            '</thead>' +
                            '<tbody class="tbody">' +
                                '<tr class="tr" ng-show="!list || list.length==0"><td class="td text-center" style="height:280px" colspan="{{cols.length+1}}">暂无数据</td></tr>' +
                                '<tr class="tr" ng-show="list && list.length > 0" ng-repeat="item in list">' +
                                    ...
                                    '<td class="td" ng-show="key.checked" ng-repeat="key in cols track by $index">' +
                                        '<div class="title" ng-show="key.isType==\'checkbox\'"> <div style="margin:auto;width:13px;line-height:13px" ng-click="key.change(item)"> <input type="checkbox" ng-model="item.status" > </div> </div>' +
                                        ...
                                        '<div class="title" ng-show="!key.isType">'+
                                            '<span ng-show="!key.dataMap" title="{{item[key.code]}}" ng-bind="item[key.code].length > 20 ? item[key.code].substring(0,15)+\'...\':item[key.code]"></span>' +
