module.exports = function(httpService, sessionFactory, $location){
    return {
        ...
        template: '<div class="table-box">'+
                    ...
                    '</ul>'+
                    '<div class="frame">'+
                        '<table class="pns-table table" ng-class="{true:\'border-b-None\',false:\'\'}[tableList.length >= 10 || !tableList || tableList.length==0]">' +
                            '<thead class="thead">'+
                                '<tr class="tr">'+
                                    ...
                                    '<th class="th" ng-show="item.checked" ng-repeat="item in cols track by $index" ng-style="{width:item.width}">'+
                                        ...
                                        '<div class="title" ng-show="item.isType==\'input\'" ng-bind="item.title"></div>' +
                                        '<div class="title" ng-show="!item.isType" ng-bind="item.title" ></div>' +
                                    '</th>'+
                                '</tr>'+
                            '</thead>' +
                            '<tbody class="tbody">' +
                                '<tr class="tr" ng-show="!tableList || tableList.length==0"><td class="td text-center not-text" style="height:280px;border:0;" colspan="{{cols.length+1}}">暂无数据</td></tr>' +
                                '<tr class="tr" ng-show="tableList && tableList.length > 0" ng-repeat="item in tableList">' +
                                    ...
                                    '</td>'+
                                    '<td class="td" ng-show="key.checked" ng-repeat="key in cols track by $index" ng-class="{true:\'borderNone\',false:\'\'}[$index+1==cols.length]" >' +
                                        ...
                                        '<div class="title" ng-show="key.isType==\'input\'" ng-class="{}">'+
                                            '<input type="text" class="pns-input" style="height:22px;" ng-model="item[key.code]" ng-keyup="key.keyup(item)">'+
                                        '</div>'+
                                        ...
                                        '<div class="title" ng-show="!key.isType">'+
                                            '<span ng-show="!key.dataMap && !key.parentId" title="{{item[key.code]}}" ng-bind="item[key.code].length > 23 ? item[key.code].substring(0,15)+\'...\':item[key.code]"></span>' +
                                            '<span ng-show="key.dataMap && !key.parentId"  title="{{key.dataMap[item[key.code]]}}" ng-bind="key.dataMap[item[key.code]]"></span>' +
                                            '<span ng-show="key.parentId"  title="{{item[key.code+\'Name\']}}" ng-bind="item[key.code+\'Name\']"></span>' +
                                        '</div>'+
                                    '</td>' +
                                '</tr>' +
                            '</tbody>'+
        ...
        link: function (scope, element, attr) {
            ...
            scope.cloumnFilter = function(cols){
                ...
                if (scope.cloumn && scope.cloumn.length > 0){
                    for(var i=0; i<scope.cloumn.length; i++){
                        ...
                        if (scope.cloumn[i].isType == "input"){
                            cols.push(scope.cloumn[i]);
                        }
            ...
            //
            scope.tableFilter = function(list,table){
                list.forEach(function(item) {
                    if(item.parentId){
                        table.forEach(function(value) {
                            value[item.code+"Name"] = item.dataMap[value[item.parentId] + "_" + value[item.code]];
                        })
                    } else if(item.dataMap){
                        table.forEach(function(value) {
                            value[item.code+"Name"] = item.dataMap[value[item.code]];
                        })
                    }
                })
            }
            ...
            scope.$watch("list",function(val){
                if(val && val.length > 0){
                    ...
                    scope.$watch("cols", function(cloumn) {
                        if(cloumn && cloumn.length > 0){
                            scope.tableFilter(cloumn,val);
                            scope.tableList = val;
                        }
                    })
                } else {
