module.exports = function(){
    return {
        ...
        template: '<div class="pns-input-search">' +
                    '<div class="label-title" ng-bind="inputTitle"></div>'+
                    '<input type="text" ng-model="inputModel" ng-click="inputClick()" ng-blur="inputBlur()" class="pns-input" placeholder="{{placeholder}}">'+
                    '<ul class="select-list" ng-show="isShow" tabindex="2">'+
                        '<li class="item" ng-repeat="item in keyList" ng-mousedown="selectChange(item)">'+
